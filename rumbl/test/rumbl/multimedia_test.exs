defmodule Rumbl.MultimediaTest do
  ...
  describe "video" do
    ...
    test "list_videos/0 returns all video" do
      owner = user_fixture()
      %Video{id: id1} = video_fixture(owner)
      assert [%Video{id: ^id1}] = Multimedia.list_video(owner)
      %Video{id: id2} = video_fixture(owner)

      assert [%Video{id: ^id1}, %Video{id: ^id2}] = Multimedia.list_video(owner)
    end

    test "get_video!/2 returns video with given id" do
      owner = user_fixture()
    ...
    test "delete_video/1 deletes the video" do
      ...
      assert Multimedia.list_video(owner) == []  
    end
